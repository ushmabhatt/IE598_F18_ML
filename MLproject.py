...
data = pd.read_csv("MLF_GP2_EconCycle_1.csv")
#Exporatory Data Analysis
print(data.describe())
del data['Date']
print(data.head())
print("number of rows in data set: ", data.shape[0], "\nnumber of columns in data set: ", data.shape[1])

 
cols = ['USPHCI','PCT 3MO FWD','PCT 6MO FWD','PCT 9MO FWD',
         'T1Y Index','T2Y Index','T3Y Index','T5Y Index','T7Y Index','T10Y Index',
         'CP1M','CP3M','CP6M',
         'CP1M_T1Y','CP3M_T1Y','CP6M_T1Y',
...
X, y3, y6, y9 = data.iloc[:, 13:].values, data.iloc[:, 1].values, data.iloc[:, 2].values, data.iloc[:, 3].values
...
print('For 3 months: MSE train: %f, For test: %f' % (MSE(y3_train, y3_train_pred),MSE(y3_test, y3_test_pred)))
print('For 3 months: R^2 train: %.3f, For test: %.3f' % (r2_score(y3_train, y3_train_pred),r2_score(y3_test, y3_test_pred)))

print('For 6 months: MSE train: %f, For test: %f' % (MSE(y6_train, y6_train_pred),MSE(y6_test, y6_test_pred)))
print('For 6 months: R^2 train: %.3f, For test: %.3f' % (r2_score(y6_train, y6_train_pred),r2_score(y6_test, y6_test_pred)))

print('For 9 months: MSE train: %f, For test: %f' % (MSE(y9_train, y9_train_pred),MSE(y9_test, y9_test_pred)))
print('9 months: R^2 train: %.3f, test: %.3f' % (r2_score(y9_train, y9_train_pred),r2_score(y9_test, y9_test_pred)))
...
#predict the value of best alpha
...
print("The best alpha value: ", alpha_space[np.argmax(ridge3_scores)])
print("The best alpha value: ", alpha_space[np.argmax(ridge6_scores)])
print("The best alpha value: ", alpha_space[np.argmax(ridge9_scores)])
...
print("The best value for K n_neighbors is: ", n_neighbors_space[np.argmax(knn3_scores)])
knn3.fit(X_train_std, y3_train)



knn6 = KNeighborsRegressor(n_neighbors=n_neighbors_space[np.argmax(knn6_scores)])
print("The best value for K n_neighbors is: ", n_neighbors_space[np.argmax(knn6_scores)])
knn6.fit(X_train_std, y6_train)

knn9 = KNeighborsRegressor(n_neighbors=n_neighbors_space[np.argmax(knn9_scores)])
print("The best value for K n_neighbors is: ", n_neighbors_space[np.argmax(knn9_scores)])
...
print('For 9 months: MSE train: %f, For test: %f' % (MSE(y9_train, y9_train_pred),MSE(y9_test, y9_test_pred)))
print('For 9 months: R^2 train: %.3f, For test: %.3f' % (r2_score(y9_train, y9_train_pred),r2_score(y9_test, y9_test_pred)))
